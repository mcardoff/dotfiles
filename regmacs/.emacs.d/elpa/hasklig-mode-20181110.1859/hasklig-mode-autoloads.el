;;; hasklig-mode-autoloads.el --- automatically extracted autoloads
;;
;;; Code:

(add-to-list 'load-path (directory-file-name
                         (or (file-name-directory #$) (car load-path))))


;;;### (autoloads nil "hasklig-mode" "hasklig-mode.el" (0 0 0 0))
;;; Generated autoloads from hasklig-mode.el

(autoload 'hasklig-mode "hasklig-mode" "\
Hasklig Ligatures minor mode.

\(fn &optional ARG)" t nil)

(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "hasklig-mode" '("hasklig-mode--")))

;;;***

;; Local Variables:
;; version-control: never
;; no-byte-compile: t
;; no-update-autoloads: t
;; coding: utf-8
;; End:
;;; hasklig-mode-autoloads.el ends here
